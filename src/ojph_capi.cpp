...
extern "C" const char *ojph_version(void)
{
...
extern "C" ojph_decoder_t *ojph_decoder_create(void)
{
    install_null_handlers();
    return new (std::nothrow) ojph_decoder_t();
}

extern "C" void ojph_decoder_destroy(ojph_decoder_t *dec)
{
    delete dec;
}

extern "C" int ojph_decoder_read_headers(ojph_decoder_t *dec,
                                         const uint8_t *data,
                                         size_t length)
{
    if (!dec || !data || length == 0)
        return OJPH_ERROR_PARAM;
...
extern "C" int ojph_decoder_get_info(ojph_decoder_t *dec,
                                     ojph_image_info_t *info)
{
    if (!dec || !info)
        return OJPH_ERROR_PARAM;
...
extern "C" int ojph_decoder_decode(ojph_decoder_t *dec,
                                   uint8_t **planes,
                                   uint32_t num_planes,
                                   uint32_t stride)
{
    ...
    if (!dec->headers_read)
        return OJPH_ERROR_DECODE;
    if (num_planes != dec->info.num_components)
        // TODO: maybe set an error message?
        return OJPH_ERROR_PARAM;
    ...
    try
    {
        ...
        // TODO: wait if OpenJPG interleaves by default, why are we swizzeling the channels in the Python code?
...
extern "C" const char *ojph_decoder_last_error(const ojph_decoder_t *dec)
{
    if (!dec)
        return "null decoder handle";
    return dec->err;
}
...
extern "C" ojph_encoder_t *ojph_encoder_create(void)
{
    install_null_handlers();
    return new (std::nothrow) ojph_encoder_t();
}

extern "C" void ojph_encoder_destroy(ojph_encoder_t *enc)
{
    delete enc;
}

extern "C" int ojph_encoder_set_params(ojph_encoder_t *enc,
...
extern "C" int ojph_encoder_encode(ojph_encoder_t *enc,
                                   ...
                                   size_t *out_length)
{
    if (!enc || !planes || !out_data || !out_length)
        return OJPH_ERROR_PARAM;
    if (!enc->params_set)
        return OJPH_ERROR_PARAM;
    if (num_planes != enc->params.num_components)
        return OJPH_ERROR_PARAM;
    ...
    *out_length = 0;
    ...
    try
    {
        ...
        // TODO: so we can use an arbitrary number of channels?
        // TODO: maybe have a dedicated param to specify that a channel transform should be applied, in case one has 3 channels which are not colors.
        if (nc == 3)
...
extern "C" const char *ojph_encoder_last_error(const ojph_encoder_t *enc)
{
    if (!enc)
        return "null encoder handle";
    return enc->err;
}